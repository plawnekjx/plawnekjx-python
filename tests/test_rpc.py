...
class TestRpc(unittest.TestCase):
    ...
    session: plawnekjx.core.Session
    ...
    def setUp(cls):
        ...
        time.sleep(0.05)
        cls.session = plawnekjx.attach(cls.target.pid)
...
""",
    ...
    def assertRaisesScriptDestroyed(self, operation):
        self.assertRaisesRegex(plawnekjx.InvalidOperationError, "script has been destroyed", operation)

    def assertRaisesOperationCancelled(self, operation):
        self.assertRaisesRegex(plawnekjx.OperationCancelledError, "operation was cancelled", operation)
