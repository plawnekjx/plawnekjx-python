...
async function start() {
  ...
  const authServiceObj = await bus.getProxyObject('re.plawnekjx.AuthenticationService15', '/re/plawnekjx/AuthenticationService');
  const authService = authServiceObj.getInterface('re.plawnekjx.AuthenticationService15');
  ...
  const hostSessionObj = await bus.getProxyObject('re.plawnekjx.HostSession15', '/re/plawnekjx/HostSession');
  const hostSession = hostSessionObj.getInterface('re.plawnekjx.HostSession15');
  ...
  let agentSessionObj = await bus.getProxyObject('re.plawnekjx.AgentSession15', '/re/plawnekjx/AgentSession/' + sessionId[0]);
  let agentSession = agentSessionObj.getInterface('re.plawnekjx.AgentSession15');

  const sink = new MessageSink('re.plawnekjx.AgentMessageSink15');
  bus.export('/re/plawnekjx/AgentMessageSink/' + sessionId[0], sink);
...
setInterval(() => {
  ...
  peerChannel.onopen = async event => {
    ...
    const peerAgentSessionObj = await peerBus.getProxyObject('re.plawnekjx.AgentSession15', '/re/plawnekjx/AgentSession');
    const peerAgentSession = peerAgentSessionObj.getInterface('re.plawnekjx.AgentSession15');

    peerBus.export('/re/plawnekjx/AgentMessageSink', sink);
...
export default function createPlawnekjxBusPlugin(): Plugin<any> {
  return (store: any) => {
    start().catch(e => {
      console.error(e);
    });
